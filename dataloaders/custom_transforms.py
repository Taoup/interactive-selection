...
class SimUserInput(object):
    ...
    def _sample_idx_(self, mask, num):
        cands = np.where(mask == 1)
        cands_indice = np.array([(x, y) for x, y in zip(cands[0], cands[1])])
...
class CropFromMask(object):
    ...
    def __init__(self, crop_elems=('image', 'gt'),
        ...
        self.jitters_bound = jitters_bound

    def __call__(self, sample):
        if self.jitters_bound:
            self.jitters = np.random.randint(self.jitters_bound[0], self.jitters_bound[1], 4)
        else:
            self.jitters = None
